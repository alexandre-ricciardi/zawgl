...
impl <'a> Page<'a> {
    ...
    fn set_list_free_page_record_ids(&self, freelist: Vec<PageRecordId>) {
        ...
        for prid in freelist {
            let beg = count * PAGE_FREE_LIST_PTR_SIZE;
            let end = beg + PAGE_FREE_LIST_PTR_SIZE;
            self.data[beg..end].copy_from_slice(&prid.to_be_bytes());
            count += 1;
        }
        for index in count..freelist_len {
            let beg = index * PAGE_FREE_LIST_PTR_SIZE;
            let end = beg + PAGE_FREE_LIST_PTR_SIZE;
            self.data[beg..end].copy_from_slice(&0u32.to_be_bytes());
        }
    }
}
...
impl Pager {
    ...
    fn next_free_record_id(&self) -> (PageId, PageRecordId) {
        let first_page = self.load_page(&0);
        let next_free_block_page = self.load_page(&first_page.get_next_freeblock_page());
