...
impl <'g0, 'g1, NID0, NID1, EID0, EID1, N0, R0, N1, R1, VCOMP, ECOMP, Graph0, Graph1>
    ...
    VCOMP: Fn(&N0, &N1) -> bool, ECOMP: Fn(&R0, &R1) -> bool {
        ...
        pub fn new(graph_0: &'g0 Graph0, graph_1: &'g1 mut Graph1, vcomp: VCOMP, ecomp: ECOMP) -> Self {
            State {
                state_0: RightMutBaseState::new(graph_0, graph_1),
                state_1: LeftMutBaseState::new(graph_1, graph_0),
                graph_0: graph_0,
                graph_1: graph_1,
                vertex_comp: vcomp,
                edge_comp: ecomp,
        ...
        pub fn feasible(&mut self, v_new: &NID0, w_new: &NID1) -> bool {
            ...
            if !(self.vertex_comp)(v, w) {
                ...
                {
                    let mut edge_predicate = MutEquivalentEdgePredicate::new(self.graph_1, |r1, r0| (self.edge_comp)(r0, r1));
        ...
        pub fn call_back<'cb, CALLBACK>(&self, mut callback: &'cb mut CALLBACK) -> bool
        where CALLBACK: FnMut(&HashMap<NID0, NID1>, &HashMap<NID1, NID0>) -> bool
        {
            callback(self.state_0.get_base_state().get_map(), self.state_1.get_base_state().get_map())
        }
}
...
impl <'g, 'a, NID, EID, N, R, RCOMP, Graph, ECOMP> MutEquivalentEdgePredicate<'g, NID, EID, N, R, RCOMP, Graph, ECOMP> 
    ...
    fn new(g: &'g Graph, ecomp: ECOMP) -> Self {
        MutEquivalentEdgePredicate {graph: g, matched_edge_set: HashSet::new(), edge_comp: ecomp,
        phantom_e: PhantomData, phantom_n: PhantomData, phantom_v: PhantomData, phantom_r_0: PhantomData, phantom_r_1: PhantomData}
    }
